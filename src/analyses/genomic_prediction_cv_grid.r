...
# ---- 10. GRID SEARCH WITH CV OVER CLADE SELECTIONS ----
message("\n=== GRID SEARCH WITH 10-FOLD CLADE CV ===")

n_cv_folds <- 10
cv_seeds <- 42:(42 + n_cv_folds - 1)
...
# Store all individual fold results
all_fold_results <- data.table(
  grid_id = integer(),
  cv_fold = integer(),
  seed = integer(),
  n_top_genes = integer(),
  ...
  sd_within_spearman = numeric(),
  pval = numeric(),
  global_spearman = numeric()
)

# Store per-clade results for each fold
all_clade_results <- data.table(
  grid_id = integer(),
  cv_fold = integer(),
  seed = integer(),
  clade = character(),
...
total_iterations <- nrow(grid) * n_cv_folds
iter_count <- 0
...
for (i in 1:nrow(grid)) {
  ...
  for (fold in 1:n_cv_folds) {
    ...
    # Select new holdout clades with this seed
    set.seed(seed)
    holdout_clades_cv <- select_holdout_clades(tree, target_frac = 0.15, min_clade = 10, max_clade = 30)
    holdout_ids_cv <- unlist(holdout_clades_cv)
    train_ids_cv <- setdiff(tree$tip.label, holdout_ids_cv)
    
    train_df_cv <- data[ID %in% train_ids_cv]
    test_df_cv <- data[ID %in% holdout_ids_cv]
    train_embeds_cv <- clean_embeds[ID %in% train_ids_cv]
    test_embeds_cv <- clean_embeds[ID %in% holdout_ids_cv]
    
    # Feature selection with these params
    combined_grid <- tryCatch({
      select_features(
        train_embeds_cv, train_df_cv, embed_cols, pheno_col,
        n_top = 1,
    ...
    if (is.null(feature_cors_grid) || nrow(feature_cors_grid) == 0) next
    ...
    fit_grid <- tryCatch({
      lm(as.formula(formula_grid), data = combined_grid, na.action = na.exclude)
    }, error = function(e) NULL)
    
    if (is.null(fit_grid)) next
    
    # Prepare test data and predict
    test_grid <- prepare_test_data(test_embeds_cv, test_df_cv, pred_cols_grid, embed_cols, pheno_col)
    ...
    # Map to clades
    id_to_clade_cv <- data.table(
      ID = unlist(holdout_clades_cv),
      clade = rep(names(holdout_clades_cv), sapply(holdout_clades_cv, length))
    )
    test_grid <- merge(test_grid, id_to_clade_cv, by = "ID")
    ...
    # Store fold results
    all_fold_results <- rbind(all_fold_results, data.table(
      grid_id = i,
      cv_fold = fold,
      seed = seed,
      n_top_genes = params$n_top_genes,
      ...
      n_predictors = length(keep_cols),
      n_train = nrow(combined_grid),
      n_test = nrow(test_grid),
      n_clades = nrow(clade_cors),
      train_r2 = summary(fit_grid)$r.squared,
      mean_within_spearman = mean(valid_cors),
      median_within_spearman = median(valid_cors),
      sd_within_spearman = sd(valid_cors),
      pval = tt_grid$p.value,
      global_spearman = global_cor
    ))
    
    # Store clade-level results
    clade_cors$grid_id <- i
    clade_cors$cv_fold <- fold
    clade_cors$seed <- seed
    all_clade_results <- rbind(all_clade_results, clade_cors[, .(grid_id, cv_fold, seed, clade, n, spearman)])
    
    # Store predictions
    pred_dt <- test_grid[, .(ID, clade, pheno, pred)]
    pred_dt$grid_id <- i
    pred_dt$cv_fold <- fold
    pred_dt$seed <- seed
    all_predictions <- rbind(all_predictions, pred_dt[, .(grid_id, cv_fold, seed, ID, clade, pheno, pred)])
    
    if (iter_count %% 50 == 0) {
      message("  Completed ", iter_count, "/", total_iterations, 
              " (grid ", i, "/", nrow(grid), ", fold ", fold, "/", n_cv_folds, ")")
    }
  }
}

# Aggregate results across CV folds
message("\n=== AGGREGATING RESULTS ACROSS CV FOLDS ===")

grid_summary <- all_fold_results[, .(
  n_folds = .N,
  n_predictors_mean = mean(n_predictors),
  n_predictors_sd = sd(n_predictors),
  train_r2_mean = mean(train_r2),
  train_r2_sd = sd(train_r2),
  within_spearman_mean = mean(mean_within_spearman),
  within_spearman_sd = sd(mean_within_spearman),
  within_spearman_median = median(mean_within_spearman),
  within_spearman_min = min(mean_within_spearman),
  within_spearman_max = max(mean_within_spearman),
  pval_median = median(pval),
  pval_min = min(pval),
  pval_max = max(pval),
  global_spearman_mean = mean(global_spearman),
  global_spearman_sd = sd(global_spearman)
), by = .(n_top_genes, n_dims_per_gene, cor_threshold)]

# Sort by mean within-clade Spearman
grid_summary <- grid_summary[order(-within_spearman_mean)]

message("\n=== TOP 20 CONFIGURATIONS (by mean within-clade Spearman) ===")
print(grid_summary[1:20])
...
message("n_predictors: ", round(best$n_predictors_mean, 1), " ± ", round(best$n_predictors_sd, 1))
message("Mean within-clade Spearman: ", round(best$within_spearman_mean, 3), 
        " ± ", round(best$within_spearman_sd, 3))
message("Range: [", round(best$within_spearman_min, 3), ", ", round(best$within_spearman_max, 3), "]")
message("Median p-value: ", format.pval(best$pval_median, digits = 3))
message("Global Spearman: ", round(best$global_spearman_mean, 3), 
        " ± ", round(best$global_spearman_sd, 3))

# Plots
p_grid <- ggplot(grid_summary, aes(x = n_predictors_mean, y = within_spearman_mean, 
                                    color = factor(cor_threshold))) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = within_spearman_mean - within_spearman_sd,
                    ymax = within_spearman_mean + within_spearman_sd), 
                width = 0, alpha = 0.5) +
  labs(
    title = "Grid Search: Within-Clade Performance vs Model Complexity",
    subtitle = paste0("10-fold CV over clade selections; error bars = ±1 SD"),
    x = "Number of Predictors (mean)",
...
# Heatmap of results
p_heat <- ggplot(grid_summary, aes(x = factor(n_top_genes), y = factor(n_dims_per_gene), 
                                    fill = within_spearman_mean)) +
  geom_tile() +
  geom_text(aes(label = round(within_spearman_mean, 2)), size = 3) +
  facet_wrap(~paste0("threshold=", cor_threshold)) +
  scale_fill_viridis_c() +
  labs(
    title = "Within-Clade Spearman by Configuration",
    x = "n_top_genes", y = "n_dims_per_gene",
    fill = "Mean r"
  ) +
  theme_minimal()

ggsave("results/phylo_cv_grid_heatmap.png", p_heat, width = 12, height = 8)

# Stability plot: SD vs mean
p_stability <- ggplot(grid_summary, aes(x = within_spearman_mean, y = within_spearman_sd,
                                         color = factor(cor_threshold),
                                         size = n_predictors_mean)) +
  geom_point(alpha = 0.7) +
  labs(
    title = "Performance vs Stability Across CV Folds",
    x = "Mean Within-Clade Spearman",
    y = "SD Within-Clade Spearman",
    color = "Threshold",
    size = "n_predictors"
  ) +
  theme_minimal()

ggsave("results/phylo_cv_stability.png", p_stability, width = 10, height = 6)

# ---- 11. SAVE ----
results <- list(
  # Initial model results (from before grid search)
  ...
  all_clade_results = all_clade_results,
...
message("\n=== RESULTS SAVED ===")
message("Total fold results: ", nrow(all_fold_results))
message("Total clade-level results: ", nrow(all_clade_results))
message("Total predictions: ", nrow(all_predictions))