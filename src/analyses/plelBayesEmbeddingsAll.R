...
taxa_df <- data[, .(ID, Order, pheno_wc2.1_2.5m_bio_8_p50)]
...
holdout_strategies <- list(
  list(name="order", get_idx=function() which(final_data$Order %in% c("Poales"))),
...
run_task <- function(row_index) {
  ...
  fit <- BGLR(y=yNA, ETA=ETA, nIter=100000, burnIn=25000, verbose=FALSE,
