...
# ---- 7. GET EMBEDDING DIMENSION COUNT ----
...
sample_embed <- read_parquet(embed_files[1])
embed_cols <- grep("^embedding_", names(sample_embed), value = TRUE)
...
# ---- 8. DIMENSION-BY-DIMENSION CORRELATION ----
...
for (i in seq_along(embed_cols)) {
  ...
  message("Processing ", dim_name, " (", i, "/", n_dims, ")")
  
  # Load ONLY this dimension from all files
  dim_data <- rbindlist(lapply(embed_files, function(f) {
    dt <- as.data.table(read_parquet(f, col_select = c("ID", "Gene", "Residue_Index", dim_name)))
    dt
  }), fill = TRUE)
  
  # Merge with features
  merged <- merge(high_cons, dim_data, 
