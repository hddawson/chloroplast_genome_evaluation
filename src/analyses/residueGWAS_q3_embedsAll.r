...
# ---- 8. GENE-SPECIFIC ASSOCIATION TESTING (NO PCA) ----
...
for (gene in genes_available) {
  ...
  for (site in gene_sites) {
    ...
    site_pheno <- merge(
      data.table(ID = taxon_embeds$ID, X),
    ...
    embed_mat <- as.matrix(site_pheno[, .SD, .SDcols = colnames(X)])
    stopifnot(ncol(embed_mat) > 0)
    
    y_resid <- site_pheno$pheno_resid
    y_raw <- site_pheno$pheno
    
    fit_resid <- tryCatch(lm(y_resid ~ embed_mat), error = function(e) NULL)
    fit_resid_null <- lm(y_resid ~ 1)
    fit_raw <- tryCatch(lm(y_raw ~ embed_mat), error = function(e) NULL)
...
saveRDS(results, "results/q3_gwas/q3_run_associations_nopca.rds")
fwrite(results, "results/q3_gwas/q3_run_associations_nopca.csv")
...
p_manhattan <- ggplot(results, aes(x = x_index, y = neg_log10_P, color = factor(gene_idx %% 2))) +
  ...
  labs(title = paste0("Q3 Run Associations (raw embeddings, ", n_pop_pcs, " pop PCs corrected)"),
       x = "Gene", y = "-log10(P)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7))

ggsave("results/q3_gwas/manhattan_q3_runs_nopca_corrected.png", p_manhattan, width = 14, height = 6, dpi = 150)
...
p_uncorr <- ggplot(results, aes(x = x_index, y = neg_log10_P_uncorr, color = factor(gene_idx %% 2))) +
  ...
  scale_color_manual(values = c("0" = "grey30", "1" = "steelblue"), guide = "none") +
  scale_x_continuous(breaks = gene_labels$x_mid, labels = gene_labels$Gene) +
  labs(title = "Q3 Run Associations (raw embeddings, uncorrected)",
       x = "Gene", y = "-log10(P)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7))

ggsave("results/q3_gwas/manhattan_q3_runs_nopca_uncorrected.png", p_uncorr, width = 14, height = 6, dpi = 150)
...
ggsave("results/q3_gwas/boxplot_by_q3_nopca.png", p_q3, width = 8, height = 6, dpi = 150)
