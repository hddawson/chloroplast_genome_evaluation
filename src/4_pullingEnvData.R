...
#process occurrences 
df <- rbindlist(list(
  fread("data/combinedOccurrences.csv"),
  fread("data/combinedOccurrences_2.csv"),
  fread("data/combinedOccurrences3.csv")
), use.names=TRUE, fill=TRUE)
summary(df$year)
hist(df$year)
sum(df$year > 2000, na.rm=TRUE)# 
df <- df[!is.na(year) & year > 2000]

a <- df[, .N, by=queryTerm]
sum(a$N>10)
#-- 17318 without filtering by year
# 16751 filtering by year, this is pretty good quality to assure 
df <- unique(df, by=c("queryTerm", "decimalLatitude", "decimalLongitude"))
#filter query term so that only alphanumeric chars remain (punctuation messes up saving)
df[, queryTerm := gsub("[^A-Za-z0-9]", "", queryTerm)]



#remove duplicate rows by queryTerm,decimalLatitude, decimalLongitude
...
saveRDS(geo_df, "data/geo_df.RDS")
#get the soil nitrogen data
quit()

df <- fread("data/geoDataOut/Topt_site/Topt_site_envData.csv")
hist(df$Topt_site)

quantiles <- df[, .(
  p10 = quantile(decimalLatitude, 0.1, na.rm=TRUE),
  p50 = quantile(decimalLatitude, 0.5, na.rm=TRUE),
  p90 = quantile(decimalLatitude, 0.9, na.rm=TRUE)
), by=queryTerm]
