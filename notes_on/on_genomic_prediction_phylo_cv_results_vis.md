    ## Run: 2026-02-10T19:57:01

    **Script**: `src/analyses/genomic_prediction_phylo_cv_results_vis.r`
    **Interpreter**: `Rscript`
    **Hash**: `88037e3a1056`

    ### Summary
    This script reloads cross-validation results from a phylogenetic machine learning analysis and recreates the best-performing model configuration to generate visualizations. It uses phylogenetic tree data, embedding features, and phenotypic data (specifically bio_8_p50 climate variable) to train a linear regression model on selected clades while holding out other clades for testing. The script performs gene-based feature selection using correlation thresholds, fits the model with the optimal hyperparameters (number of top genes, dimensions per gene, and correlation threshold), and creates a scatter plot comparing predicted vs observed values that are z-scored within each holdout clade. The analysis evaluates model performance using within-clade Spearman correlations to assess how well the model generalizes across different phylogenetic groups.
...
`geom_smooth()` using formula = 'y ~ x'
    ...
    **Hash**: `3d34955b392a`
    ...
    ### Summary
    This script evaluates the best-performing phylogenetic cross-validation model by reloading previously saved grid search results and re-running the optimal configuration. It performs clade-based cross-validation where specific phylogenetic clades (10-30 species each) are held out as test sets, then uses protein embedding features combined with linear regression to predict a climate variable (bio_8_p50 - mean temperature of wettest quarter). The model applies gene-specific feature selection based on correlation thresholds, fits a linear model on training data, and evaluates performance by calculating within-clade Spearman correlations between predicted and observed values after z-score scaling. Finally, it generates scatter plots and heatmaps visualizing the within-clade prediction accuracy, with the results showing how well the model generalizes to evolutionarily distant species groups.
...
`geom_smooth()` using formula = 'y ~ x'
`geom_smooth()` using formula = 'y ~ x'
Error in LSD::heatscatter(plot_data$pred_scaled, plot_data$pheno_scaled,  : 
  First two argument must be numeric vectors!
Execution halted
