...
edge_order <- order(ftree$edge.length, decreasing = TRUE)
child_nodes <- ftree$edge[edge_order, 2]
tip_edges <- child_nodes[child_nodes <= Ntip(ftree)]


tip_ids <- ftree$tip.label[tip_edges]
...
gymno_in_tree <- gymno_ids[gymno_ids %in% ftree$tip.label]

cat("Total gymno tips:", length(gymno_in_tree), "\n")

# Check if gymnosperms together are monophyletic
is_gymno_mono <- is.monophyletic(ftree, gymno_in_tree)
cat("Are all gymnosperms together monophyletic?", is_gymno_mono, "\n")

gymno_mrca <- getMRCA(ftree, gymno_in_tree)
ftree_rooted <- root(unroot(ftree), node = gymno_mrca,keep.root.edge=TRUE)
is.rooted(ftree_rooted)

set.seed(123)
subtree <- keep.tip(ftree_rooted, sample(ftree_rooted$tip.label, 1000))
...
poales_tree <- drop.tip(ftree, setdiff(ftree$tip.label, poales_in_tree))
...
plot(poales_tree, cex = 0.3, no.margin = TRUE)
dev.off()
